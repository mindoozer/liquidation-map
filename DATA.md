...
- `magnet.json` — `generatedAt · horizonH · firstSnapshot/lastSnapshot (stamped only from snapshots that passed the kline-coverage gate — the window actually scored, NOT the oldest snapshot on disk) · totalWalls · tokens{SYM:{samples}} · pooled[{bucket, n, touch, base, lift}] · cells[{tok, side ('above'|'below' spot), bucket, n, touch, base, lift}]`. Forward magnet test; `pooled` is a token-mix — read `cells` before trusting a pooled shape. **Diagnostic, not a thesis (decision 2026-09-16):** lift answers "was this wall where price was going anyway?" per wall; the far-wall verdict is base-rate-window-dependent and does not converge with more months — see the skill's `magnets` mode. **Era note 2026-09-16:** the kline archive merged this day; pooled numbers stepped (5–10% lift −4.5 → ≈+2 pp, walls ≈52k → ≈106k, `cells` appeared) because the scored window widened from a rolling ~29d to 2026-07-16→now, not because anything in the market changed.
