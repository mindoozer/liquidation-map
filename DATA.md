...
- **Gitignored** (regenerated/accrued): `data.json`, `snapshots/`, `liquidations/`, `magnet.json`, `calibration.json`, `*.log`, `data/` (health sentinel `health-err.txt` — current tick only, deleted by the next clean fetch — plus `health-hist.txt`, append-only timestamped failure lines, 7-day retention). **In git:** `config.yml`, code, `sql/`, `DATA.md`.
