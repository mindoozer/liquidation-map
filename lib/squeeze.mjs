...
export function computeSqueeze(map, tokenData, liqDir, symbol, nowMs = Date.now()) {
  ...
  let etfUsdM = null, etfDate = null;
  try {
    const f2 = new URL('../etf-flows.json', liqDir);
    if (existsSync(f2)) { const e = JSON.parse(readFileSync(f2, 'utf8')).flows?.[symbol]; if (e && isFinite(e.usdM)) { etfUsdM = e.usdM; etfDate = e.date; } }
  } catch {}
  ...
  if (etfUsdM != null && etfThresh > 0) { const u = etfUsdM * 1e6; score += u > etfThresh ? -1 : u < -etfThresh ? 1 : 0; } // inflow → short-squeeze
  ...
  return { funding8h: f, fundingVenues: map.fundingVenues, fuelL, fuelS, ratio, realizedLong, realizedShort, longShare, hourlyVol, absorb, etfUsdM, etfDate, score, dir, level };
}
