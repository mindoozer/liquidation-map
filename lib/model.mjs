...
// Per-side spatial CONCENTRATION in offset space (|Δ%| from price), normalized to that
// side's OWN mass — so it's pure shape, decoupled from the magnitude split (longFrac).
// "How detonable is this side": mass piled tight near price = low ignition barrier +
// reflexive cascade (and a price magnet); the same notional smeared far out = high barrier,
// weak pull. Computed on the FULL compute-range bins, so it's independent of the auto-fit
// display window. DIAGNOSTIC only — not fed into any score (gated on real-liq validation).
function concentrationProfile(bins, P, sideKey) {
  const arr = [];
  let total = 0;
  for (const b of bins) {
    const m = b[sideKey] || 0;
    if (m <= 0) continue;
    arr.push({ off: Math.abs(b.price - P) / P, m });
    total += m;
  }
  if (total <= 0) return null;
  arr.sort((a, b) => a.off - b.off); // nearest-to-price first
  let acc = 0, half = null, near2 = 0, near5 = 0, cen = 0, peak = 0, peakOff = 0;
  for (const { off, m } of arr) {
    if (off <= 0.02) near2 += m;
    if (off <= 0.05) near5 += m;
    cen += off * m;
    if (m > peak) { peak = m; peakOff = off; }
    acc += m;
    if (half == null && acc >= 0.5 * total) half = off;
  }
  const pc = (x) => +(x * 100).toFixed(2);
  return {
    nearFrac2: +(near2 / total).toFixed(3),       // share of this side's mass within ±2% of price
    nearFrac5: +(near5 / total).toFixed(3),
    halfMassPct: half != null ? pc(half) : null,  // |offset| holding the nearer 50% of the side's mass
    centroidPct: pc(cen / total),                 // mass-weighted mean |offset|
    peakOffPct: pc(peakOff),                       // |offset| of the single densest bin
    peakShare: +(peak / total).toFixed(3),         // that bin's share of the side (peakedness)
    label: half == null ? null : (half <= 0.025 ? 'TIGHT' : half <= 0.07 ? 'MIXED' : 'SPREAD'),
  };
}
...
export function buildMap(tokenData, config, tokenCfg = {}) {
  ...
  // --- per-side spatial concentration (offset space, full compute range) — DIAGNOSTIC, not scored ---
  // The squeeze score only ever compares per-side TOTALS (fuel within ±5%); it can't tell a
  // tight near-price wall from the same notional smeared far out — yet that shape governs which
  // side actually cascades. Surface it so the asymmetry is visible & loggable for validation.
  const concLong = concentrationProfile(bins, P, 'totalLong');
  const concShort = concentrationProfile(bins, P, 'totalShort');
  let ignition = null;
  if (concLong?.halfMassPct != null && concShort?.halfMassPct != null) {
    const dl = concLong.halfMassPct, ds = concShort.halfMassPct;
    // smaller half-mass = mass sits closer = lower barrier to a cascade on that side.
    // short side closer → an upside rip ignites sooner; long side closer → a downside flush.
    ignition = {
      longHalfPct: dl, shortHalfPct: ds,
      gapPct: +Math.abs(dl - ds).toFixed(2),
      closerSide: Math.abs(dl - ds) < 0.5 ? 'symmetric' : (ds < dl ? 'short' : 'long'),
    };
  }
  const concentration = { long: concLong, short: concShort, ignition };
  ...
  return {
    ...
    concentration,                // per-side spatial concentration {long,short,ignition} — diagnostic, NOT scored
    venues: venues.map((v) => ({ exchange: v.exchange, symbol: v.symbol, markPrice: v.markPrice, openInterestUsd: v.openInterestUsd, stale: v.stale || false, staleAsOf: v.staleAsOf || null })),
    staleVenues: venues.filter((v) => v.stale).map((v) => v.exchange),
    tiers,
    bins: view,
    maxBinTotal,
    range: { lo, hi },
